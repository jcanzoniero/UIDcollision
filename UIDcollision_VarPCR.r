#### Simulation of UID collisions with variable PCR ##############
...
# Incorporates variation in PCR success according to UID 
...
# For additional information, please see Using UIDcollision_VarPCR,r by Jenna VanLiere Canzoniero

UIDerrorVarPCR<-function(nsamp,mutFreq,nUID,seed=1,conFrac=0.95,PCRcyc=25, PCRsuc=c(0.25,0.5), nb, minMembers=2){
  ...
  #From the UIDs that bound to sample molecules, pick nb UIDs which will have higher PCR success rate
  better<-unique(UIDs)[1:nb]
  
  #determine the indices of the UIDs which have higher PCR success rate
  inds<- which(UIDs %in% better)
  
  #simulate the lower PCR success rate for all samples usin binomial distributed random numbers
  amp<- rbinom(nsamp,PCRcyc,min(PCRsuc))
  
  #simulate the higher PCR success rates for samples with better UIDs and replace those in amp vector
  highamp<-rbinom(length(inds), PCRcyc, max(PCRsuc))
  amp[inds]<-highamp
...
#output file
of<-"UIDerrorVarPCRsuc2and8example.txt"
mutFreq<-c(0.0001, 0.001,0.01,0.1,0.2,0.5) #mutation frequency
...
for (m in mutFreq) {
  for (ns in nsamp){
    for (t in taglength){
      ...
      ec<-ecol(ns,u)
      for (s in seed){
        result<-UIDerrorVarPCR(nsamp=ns,mutFreq=m,nUID=u,seed=s, PCRsuc=c(0.2, 0.8), conFrac=0.95, nb=(3*t+1))
        out[i,1]<-m
        out[i,2]<-ns
        out[i,3]<-u
        out[i,4]<-p
...
sink(of)
cat(c("PCRsuc 0.2 0.8 conFrac 0.95 PCRcyc 25 minMembers 2 nb 3t+1\n"))
sink()
write.table(out, file=of, quote=FALSE, col.names=TRUE, append=TRUE)
